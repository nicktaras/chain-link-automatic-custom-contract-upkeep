...
/**
 * @title The PayWizard contract
 * @notice A contract compatible with Chainlink Automation Network that monitors and funds eth addresses
 * This contract has been extended upon by PayWizard to allow for managment of accounts listed.
 */
    ...
    AutomationCompatibleInterface
{
     ...
     */
    function addAddress(address newAddress, uint96 minBalanceWei, uint96 topUpAmountWei) external onlyOwnerOrForwarder {
        require(newAddress != address(0), "Address cannot be zero");
        require(!s_targets[newAddress].isActive, "Address already exists");

        s_targets[newAddress] = Target({
            isActive: true,
        ...
        s_watchList.push(newAddress);
    }

    /**
     * @notice Removed an address
    */
    function removeAddress(address addressToRemove) external onlyOwnerOrForwarder {
        require(s_targets[addressToRemove].isActive, "Address does not exist");

        // Remove address from watchlist
        for (uint256 i = 0; i < s_watchList.length; i++) {
            if (s_watchList[i] == addressToRemove) {
        ...
        delete s_targets[addressToRemove];
    }
    ...
    modifier onlyOwnerOrForwarder() {
        if (msg.sender != s_forwarderAddress && msg.sender != owner()) {
            revert OnlyForwarder();
        }
        _;
    }
